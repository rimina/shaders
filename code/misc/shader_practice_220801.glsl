...
float box(vec3 p, vec3 b){
  vec3 d = abs(p)-b;
  return length(max(d, 0.)) + min(max(max(d.x, d.y), d.z), 0.0);
}

float scene(vec3 p){
  vec3 pp = p;
  
  float x = length(p)-2.0;
  
  float bb = box(pp-vec3(0.0, -5., 0.), vec3(10., 0.5, 10.));
  
  for(int i = 0; i < 7; ++i){
    pp = abs(pp)-vec3(0.25, 0.55, 0.4);
    rot(pp.xz, time*0.01+fft*10.);
    rot(pp.xy, time*0.5+fft*2.);
    rot(pp.yz, time*0.25);
  }
  
  float cx = length(pp)-0.5;
  float xx = length(pp)-0.2;
  
  vec3 g = vec3(0., 0.3, 0.4) * 0.1 / (abs(xx)+0.1);
  g += vec3(0.8, 0.2, 0.0) * 0.09 / (abs(cx)+0.05);
  g += vec3(0., 0.5, 0.8) * 0.01 / (abs(x)+0.1);
...
vec3 shade(vec3 p, vec3 rd, vec3 ld){
  ...
  return vec3(0.1)*l + vec3(0.8, 0.6, 0.3)*s;
}
...
void main(void)
{
  ...
  vec3 ro = vec3(18.0 * cos(time*0.1), 2.0, 18.0 * sin(time* 0.1));
