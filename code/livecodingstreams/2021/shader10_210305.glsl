...
const vec3 AMBIENT = vec3(0.8, 0.6, 0.5);
const vec3 LC = vec3(0.8, 0.0, 0.2);
const vec3 LC2 = vec3(0.4, 0.7, 0.9);
...
Material getSphereMaterial(vec2 index){
  Material mat;
  
  vec3 col = palette(length(index)*0.5,
                       vec3(0.2, 0.1, 0.25),
                       vec3(0.5, 0.5, 0.5),
                       vec3(1., 1., 0.),
                       vec3(0.25, 0.5, 0.8));
  ...
  mat.lambertian = col;
  mat.specular = col*vec3(0.8, 0.3, 0.5);
...
Material getGroundMaterial(){
  ...
  mat.reflective = false;
  mat.refractive = false;
  mat.lambertian = vec3(0.0, 0.3, 0.5);
  mat.specular = vec3(0.0, 0.5, 1.0);
...
float scene(vec3 p, out Material mat){
  ...
  float maa = plane(p, vec3(0.0, 1.0, 0.0), 1.0);
  
  vec2 ido = pMirrorOctant(pp.xz, vec2(4.0, 12.0));
  vec2 ids = pMod2(pp.xz, vec2(10.0, 13.0));
  ...
  float laatikko = box(pp-vec3(1.0, 1.5, 1.0), vec3(0.5, 3.0+height, 1.25));
  
  pp -= vec3(0.1, 4.5+height, 0.0);
  rot(pp.xy, radians(60.));
  
  float katto = box(pp, vec3(0.1, 2.0, 3.0));
  
  laatikko = fOpUnionStairs(laatikko, katto, 0.5, 8.0);
  
  float guard = -fBoxCheap(pp, vec3(6.0, 10.0, 10.0)*0.5);
...
vec3 shade(vec3 p, vec3 dir, vec3 ld, vec3 lp, Material mat){
  ...
  vec3 sp = vec3(0.0);
  /*Material sm;
  bool hit = march(p+E*n*2.0, ld, sp, false, sm);
  float shadow = 1.0;
  if(hit){
    shadow = 0.2;
  }*/
  
  return (lamb*mat.lambertian*mat.l_intensity + spec*mat.specular*0.5);//*shadow;
...
vec3 fog(vec3 col, vec3 p, vec3 ro, vec3 rd, vec3 ld, vec3 lc){
  ...
  float fa = 1.0-exp(-d*0.03);
  vec3 fc = mix(vec3(0.6, 0.3, 0.2), lc, pow(sa, 4.0));
...
void main(void)
{
  ...
  //vec3 ro = vec3(10.0*sin(time*0.5), 14.0, 10.0*cos(time*0.5));
  //vec3 rt = vec3(0.0, 4.0, 0.0);
  vec3 ro = vec3(-4.0, 3.0+cos(time*0.1), time*2.0);
  vec3 rt = vec3(10.0*sin(fft*10.5)*0.4, 4.0, 5.0+ro.z);
  ...
  Material mat = getGroundMaterial();
  float t = marchP(ro, rd, mat);
  vec3 p = ro+rd*t;
  
  vec3 col = vec3(0.0);
  
  vec3 lp = ro - vec3(0.0, 80.0, 0.0);
  vec3 lt = rt - vec3(0.0, 0.0, 80.0);
  vec3 ld = normalize(lt-lp);
  
  vec3 lp2 = vec3(ro.x+40.0*sin(time*0.5), ro.y, ro.z+40.0*cos(time*0.5));
  vec3 lt2 = ro;
  ...
  if(t < FAR){
    ...
    if(mat.reflective){
      ...
      vec3 pr = p;
      Material refmat = getGroundMaterial();
