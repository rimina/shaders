...
float scene(vec3 p){
  ...
  vec3 pp = p;
  rot(pp.xy, time*0.5);
  rot(pp.yz, time*0.25*fft*0.01);
  
  float tunneli = -box(pp, vec3(3.0, 4.0, 3.0));
  
  
  
  float pallo = sphere(pp-vec3(0.0, 1.0, 0.5), 1.00);
  
  
  material = 0;
  if(pallo < tunneli || pp.y < -1.5 || pp.y > 1.5){
    material = 1;
  }    
  
  return min(tunneli,pallo);
}
...
vec3 march(vec3 rd, vec3 ro, vec3 ld, out bool hit, out vec3 p){
  ...
  if(t < FAR){
    if(material == 1){
      ...
      float m = mod(p.y-time*sin(fft*0.5), 4.0) - 0.5;
      if(m > 0.0 && m < 2.0){
        col = col.bgr;
      }
      else if( m < 0.0 && m > -2.0){
  ...
  //col += (t*0.1 * vec3(0.5*fft*0.1, 0.2, 0.6));
...
void main(void)
{
  ...
  vec3 ro = vec3(sin(time*0.5), 2.0, cos(time*0.25));
  ...
  vec3 rd = normalize(mat3(x, y, z) * vec3(q, radians(30.0)));
  ...
  col += (vec3(0.8, 0.8, 0.5)-prev)*0.5;
  //col = mix(col, vec3(1.0, 1.2, 0.9)-prev, 0.5);
  vec3 n = normals(p);
  prev = texture(texPreviousFrame, uv*n.xy).rgb;
  col = mix(col, prev, 0.5);
  prev = texture(texPreviousFrame, uv).rgb;
  col = mix(col, prev, vec3(0.5, 0.2, 1.0));
